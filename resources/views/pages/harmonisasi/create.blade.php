...
@section('content')
    <div class="p-5 lg:p-14 lg:py-7">
        ...
        <div id='container'>
            <form action="{{ route('harmonisasi.store_new') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="p-5 mb-5 grid gap-5 shadow border border-slate-300 border-t-8  border-t-sky-700 rounded-md overflow-hidden bg-white">
                    <input type="text" name="uu" value="Rancangan Undang-Undang" class="w-full font-semibold border-0 border-b-2 border-b-slate-400 focus:ring-0 p-2 text-2xl" required>
                    <input type="text" name="tentang" placeholder="Tentang" value="Pemerintahan Digital" class="w-full h-5 border-0 border-b-2 border-b-slate-400 overflow-y-auto focus:ring-0 p-2 pt-0" required>
                    <input type="text" name="keyword" placeholder="Keyword" class="w-full h-5 border-0 border-b-2 border-b-slate-400 overflow-y-auto focus:ring-0 p-2 pt-0" required>
                    <input type="hidden" name="fromFileUpload" value="{{ isset($fromFileUpload) ? 'true' : 'false' }}">
                    @isset($fromFileUpload)
                        <a href="{{ asset('assets/hitung/temp-harmonisasi.pdf') }}" target="blank" class="text-xl text-sky-600">
                ...
                <div class="flex justify-center">
                    <button class="px-3 py-2 font-medium text-lg bg-sky-800 rounded text-white">Hitung</button>
                </div>
            </form>
        </div>
    </div>
